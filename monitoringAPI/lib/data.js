  ...
  fs.open(
    ...
    (errorOpening, fileDescriptor) => {
      if (!errorOpening && fileDescriptor) {
        ...
        callback(
          "Error opening file.File may already exists.Cant register with same number."
        );
      }
    }
  );
...
lib.makeList = (dir, callback) => {
  fs.readdir(`${lib.baseDir}${dir}`, (errorReading, fileNameList) => {
    if (!errorReading && fileNameList && fileNameList.length > 0) {
      let trimmedFileNameList = [];
      fileNameList.forEach((filename) => {
        trimmedFileNameList.push(filename.replace(".json", ""));
      });
      callback(errorReading, trimmedFileNameList);
    } else {
      callback("Could not read the directory.");
    }
  });
};
